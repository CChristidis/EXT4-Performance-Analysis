...
echo "$2, $3, $4, $5, $filesize, $filesize_with_mean, $meanfilesize, $nthreads, $nshadows, $ndbwriters" >> /root/Desktop/test.txt

# we'll use this later, maybe in the final script, right before we plot stuff.
# echo "$(column -s, -t --table-columns 95%_conf_interval,std_dev,mean,throughput,filesize,filesize_mean,meanfilesize,nthreads,nshadows,ndbwriters /root/Desktop/test.txt)" > /root/Desktop/test.txt
